...
runScriptForwarding TestSetup{..} msgCountersRef msgsRef tracerGetter = do
  ...
  forAll generator \(scripts :: Vector Script) -> ioProperty do
      ...
      for_ scripts''' \script''' ->
        forkChild children (playIt script''' tr 0.0)
      resErr <- lefts <$> waitForChildren children []
      if not (null resErr)
        then throw (head resErr)
          ...
          in if numMsg > 0 then do
            ...
            let parsedMessages :: Vector Message
                parsedMessages = Vector.fromList $ rights (map (prs . pack) (lines contents))
            ...
            pure $ conjoin
              ...
              , counterexample "Messages do not match the Messages do not match." do
                  Utils.vectorSort parsedMessages == Utils.vectorSort totalMsgs
              ]
          else
            pure (property True)
