...
import           Control.Concurrent.MVar (newMVar, putMVar, readMVar, tryReadMVar)
...
modifyRegistry_ :: Registry a b -> (Map a b -> IO (Map a b)) -> IO ()
modifyRegistry_ (Registry registry) = modifyMVar_ registry

readRegistry :: Registry a b -> IO (Map a b)
